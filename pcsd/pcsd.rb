...
def __msg_cluster_name_already_used(cluster_name)
  return t.post.add_same_cluster(cluster_name)
end

def __msg_node_name_already_used(node_name, cluster_name)
  return t.post.add_same_node(node_name, cluster_name)
end

def __msg_sync_conflict_detected(please_repeat=nil)
  if not please_repeat
    please_repeat = t.post.Please_repeat_the_last_action_if_appropriate
  end
  return t.post.Configuaration_conflict_detected(please_repeat)
end

def __msg_sync_nodes_error(node_name_list)
  return t.post.Unabled_to_save_settings(node_name_list.join(', '))
end
