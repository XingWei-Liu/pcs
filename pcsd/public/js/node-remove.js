...
nodesRemove.dialog.create = function(){
  ...
  verify_remove(
    ...
    translate("Remove Node(s)"), // ok_text
    translate("Remove Node"), // title
    nodeNameList,
    {
      closeOnEscape: false,
  ...
  api.clusterNodeRemove(
    ...
    },
    {
      confirm: function(msgs){
        return tools.submit.confirmForce(translate("remove nodes from cluster"), msgs);
      },
      buildMsg: function(report){
        switch(report.code){
          case "CANNOT_REMOVE_ALL_CLUSTER_NODES":
            return (
              translate("No nodes would be left in the cluster. If you intend to destroy the whole cluster, go to cluster list page, select the cluster and click 'Destroy'.")
            );
        }
      },
    },
    ...
    switch(rejectCode){
      ...
      case api.err.NODES_REMOVE.PCS_LIB_EXCEPTION:
        alert(translate("Server returned an error: ")+data.msg);
        nodesRemove.dialog.close();
        break;
