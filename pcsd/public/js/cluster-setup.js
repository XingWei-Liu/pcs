...
clusterSetup.data.validateNameAndNodes = function(formData){
  ...
  if(formData.clusterName == ""){
    errors.push({
      type: "error",
      msg:translate("You may not leave the cluster name field blank."),
    });
  }
  if(formData.nodesNames.length == 0){
    errors.push({
      type: "error",
      msg:translate("At least one valid node must be entered."),
    });
  }
  return errors;
};
...
clusterSetup.step.clusterNameNodes = function(){
  clusterSetup.step.set({
    ...
    title: translate("Create cluster: Cluster name and nodes"),
    buttons: [
      {
      ...
      {
        text: translate("Go to advanced settings"),
...
clusterSetup.dialog.addNode = function(){
  ...
  $("td:first-child", newNode).text(translate("Node ")+(nodes.length+1)+":");
