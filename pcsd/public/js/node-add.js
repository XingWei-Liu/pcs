...
nodeAdd.validateFormData = function(formData){
  ...
  if(formData.nodeName == ""){
    errors.push({
      type: "error",
      msg:translate("You may not leave the node name field blank."),
    });
  }
  return errors;
};
    ...
    return api.clusterNodeAdd(
      { nodeAddData: formData, clusterName: clusterName },
      {
        confirm: function(msgs){
          return tools.submit.confirmForce(translate("add node to cluster"), msgs);
        },
      },
    );
